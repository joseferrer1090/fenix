...
function current_fund() {
    ...
    $income = $db->query("SELECT SUM(nominal) as funds FROM fund_transaction WHERE to_id = :to_id AND type <> 6;");
    // Get The Expense
    $db->bind("from_id", $_SESSION["uid"]);
    $expense = $db->query("SELECT SUM(nominal) as expense FROM fund_transaction WHERE from_id = :from_id  AND type <> 6;");
...
function current_register_fund() {
    ...
    $db->bind("from_id", $_SESSION["uid"]);
    $db->bind("to_id", $_SESSION["uid"]);
    $available = $db->query("SELECT SUM(nominal) as funds FROM fund_transaction WHERE (type = 6 AND from_id = :from_id) OR (type = 10 AND to_id = :to_id);");
...
function current_utilities(){

    global $db;
    // Get The register fund
    $db->bind("to_id", $_SESSION["uid"]);
    $income = $db->query("SELECT SUM(nominal) as funds FROM fund_transaction WHERE to_id = :to_id AND type_fund=3;");
    $available = $income[0]["funds"];
    if($available){
        return $available;
    }else{
        return "0";
...
function xNumberToken($length = 6) {
    $chars = "0123456789";
...
function userExist($uname) {
    global $db;
    $db->bind("uname", $uname);
    $row = $db->query("SELECT * FROM user_id WHERE uname = :uname and banned = 0");
...
function getFechaExpiracion($id){
     global $db;
     date_default_timezone_set('America/Bogota');

     if($id>0){
     $sql="SELECT days FROM product WHERE product_id=$id";
     $row=$db->query($sql);
     $dias=$row[0]['days'];  
     $registro= get_data($_SESSION['uid'], 'register_date');
     $fecha=date('Y-m-d',strtotime("+$dias days",strtotime($registro)));
